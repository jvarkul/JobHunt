...
sample_jobs AS (
    ...
    ) AS jobs(company_name, job_title, description, application_link)
    CROSS JOIN user_ids
)
...
FROM user_ids, (VALUES
    ...
    ('Tech Consulting Group', 'Full Stack Engineer', 'Recruiter outreach, NDA signed. Enterprise projects.'),
    ('Stealth Startup', 'Senior Developer', 'Equity heavy compensation package. Early stage opportunity.'),
    ('Remote Tech Co', 'Software Engineer', 'Networking connection, informal application process.')
) AS jobs(company_name, job_title, description)
ORDER BY RANDOM()
LIMIT 15;
